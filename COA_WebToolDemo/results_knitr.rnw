...
cat('\\renewcommand\\TPTminimum{5in}\n')
cat('\\begin{TableNotes}[flushleft]\n')
cat('\\footnotesize')
...
cat('\\toprule\n')
cat('\\rowcolor[gray]{.8}{Terrestrial and Wetland Habitat\\tnote{a} & Area(ac)} \\\\ \n')
cat('\\midrule\n')
cat('\\endhead\n')
for(m in macros){
...
if(nrow(aoi_HabLotic) == 0){
  cat('\\hline\n')
  cat('\\rowcolor[gray]{.8}{Streams and River Habitat\\tnote{a} & Length(mi)} \\\\ \n')
  cat('\\hline\n')
