...
cat('\\toprule\n')
cat('SGCN Name & Season & CAT1 & CAT2 & CAT3 & CAT4 & Occurrence Probability & Specific Habitat Requirements \\\\ \n')
cat('\\midrule\n')
cat('\\endhead\n')
for(t in taxa){
  if(t=="Bird"){
  ...
  for(i in 1:nrow(sgcn)){
    if(sgcn$TaxaGroup[i] == t){
      cat(paste0(paste(sgcn$SCOMNAME[i],"\\newline \\hspace{3mm} (\\textit{",sgcn$SNAME[i],"})",sep=""),'&',sgcn$SeasonCode[i],'&',sgcn$CAT1_glbl_reg[i],'&',sgcn$CAT2_com_sp_com[i],'&',sgcn$CAT3_cons_rare_native[i],'&',sgcn$CAT4_datagaps[i],'&',sgcn$OccProb[i],'&',sgcn$SpecificHabitatRequirements[i],'\\\\ \n'))
    }
  }
}
cat('\\bottomrule\n')
...
\vspace{10mm}
\noindent\Large\textbf{Research Needs} \\
\noindent\normalsize The following table includes research needs for the SGCN present at the site. Please note that the specific research needs may not be applicable at this particular AOI. 
<<label=longtable3, echo=FALSE, results='asis'>>=
research <- aoi_Research
...
#action_cat <- unique(actions$ActionCategory1)
cat('\\begin{longtable}{p{2.5in}p{7in}}\n')
cat('\\toprule\n')
cat('species & research needs \\\\ \n')
cat('\\midrule\n')
cat('\\endhead\n')
#for(a in action_cat){
#  cat(paste0('\\multicolumn{1}{l}{\\rowcolor[gray]{.7}',a,'}', '\\\\ \n'))
  for(i in 1:nrow(research)){
    #if(actions$ActionCategory1[i] == a){
      cat(paste0(research$SCOMNAME[i],'&',research$ResearchQues_Edited[i],'\\\\ \n'))
...
@
\vspace{10mm}
\noindent\Large\textbf{Survey Needs} \\
\noindent\normalsize The following table includes survey needs for the SGCN present at the site. Please note that the specific survey needs may not be applicable at this particular AOI. 
<<label=longtable4, echo=FALSE, results='asis'>>=
survey <- aoi_Survey
cat('\\begin{longtable}{p{2.5in}p{7in}}\n')
cat('\\toprule\n')
cat('species & research needs \\\\ \n')
...
#  cat(paste0('\\multicolumn{1}{l}{\\rowcolor[gray]{.7}',a,'}', '\\\\ \n'))
  for(i in 1:nrow(survey)){
    #if(actions$ActionCategory1[i] == a){
      cat(paste0(survey$SCOMNAME[i],'&',survey$NumSurveyQuestion_Edited[i],'\\\\ \n'))
      #cat(paste0('\\hspace{5mm}', survey$SCOMNAME[i], '\\\\ \n'))
    #}
  }
#}
cat('\\bottomrule\n')
