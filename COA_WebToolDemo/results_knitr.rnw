...
\begin{center}
\vspace{-10mm}
\framebox(450,175){**Placeholder for habitat map**}
\end{center}
...
cat('\\begin{longtable}{p{0.6in}p{6in}L{2.4in}}\n')
cat('\\toprule\n')
...
\vspace{2mm}
\noindent\normalsize 
For additonal information about the SGCN and Conservation Actions in this report, please contact:
\begin{itemize}
...
\item For fish, reptiles and amphibians, and freshwater mussels...
\end{itemize}
\vspace{2mm}
\noindent\normalsize 
For other inquires in regard to this tool or the Pennsylvania Wildlife Action Plan, please contact:
