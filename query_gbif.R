...
dat <-  dat[dat!="no data found, try a different search"] # doesn't work for one species
# turns the list to a data frame
datdf <- ldply(dat)

#subset out the records from the eBird dataset, using the ebird dataset key. Unfortunately, we need to download them first.  Might be a better way to do this somewhere.
datdf <- subset(datdf,datasetKey!='4fa7b334-ce0d-4e88-aaae-2e0c138d049e')
...
setnames(inverts, "Scientific_Name", "SNAME")
gbifdata <-  join(gbifdata,inverts,by=c('SNAME'))
